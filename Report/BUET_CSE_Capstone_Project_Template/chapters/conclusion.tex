...
\begin{xltabular}{\textwidth}{@{}>{\hsize=0.42\hsize}Y>{\hsize=1.40\hsize}Y>{\hsize=1.18\hsize}Y@{}}
  ...
  O3 Autonomous execution on two platforms with three-way attribution &
    ...
    Met, and measured. Autonomy reached 100\,\% on Google Contacts in August and 0\,\% of the DataGhurhi web campaign's 64 test cases were lost to the agent. On Shobar Khamar, the harder production target, it settled at 69\,\% (\Cref{fig:campaigns}). \\
  \addlinespace
  ...
  O7 Measured cost within budget &
    ...
    Met. USD 0.008 per executed test, about one taka, reconciled against the provider's billing record over 88 device executions rather than derived from a price card (\Cref{sec:budget}). Total metered spend for the project was under USD 50. \\
  \addlinespace
  O8 Operator surface &
...
drove campaigns against two Android applications, Google Contacts and the
production marketplace Shobar Khamar, and against three web targets: a
departmental survey platform, a public practice site and a deliberately faulty
demonstration application. Five targets carried a full campaign and are
reported in \Cref{tab:targets}; further targets were configured but not run to
completion and are not claimed here. Adding a target is a validated
configuration file, not a code change.
...
The evidence base is a set of campaigns across the five targets named above
...
A fourth boundary emerged from the measurements themselves and is recorded
here because it qualifies the transfer claim above. Autonomy does not carry
across targets unchanged: it reached 100\,\% on the application the agent was
tuned against and 69\,\% on the production marketplace, and the gap is
attributable to state re-identification rather than to anything in the planner
(\Cref{sec:revisions}, D4). The system recognises interface states it has not
seen before far more reliably than it recognises ones it has, which inflates
state counts and deprives navigation memory of the repetition it depends on.
Anyone reading the transfer result should read it as transfer of the
\emph{architecture} rather than of the \emph{tuning}.
