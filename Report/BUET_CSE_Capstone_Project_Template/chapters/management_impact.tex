...
The project had no procurement budget and no institutional compute grant
(constraint C2). What it needed in place of money was identified at the outset
as a short resource list: a device to test on, an application with real
behaviour, a specification to test it against, and metered model inference. The
first three were obtained without cash. The fourth is the project's only
expenditure, and it is small enough to be reported exactly rather than
estimated.

\Cref{tab:budget} lists each resource and how it was obtained. One convention
governs it and is worth stating, because it determines the total: a resource
the project did not pay for is recorded at zero, not at an imputed market
price. Assigning notional prices to already-owned laptops, to a donated handset
and to the team's own labour would produce a larger and more impressive figure,
but not a truer one, and it would bury the single number here that a reader can
act on. Conversions use an approximate rate of BDT 120 to the United States
dollar.

\begingroup
\small
\begin{xltabular}{\textwidth}{@{}>{\hsize=0.60\hsize}Y>{\hsize=1.14\hsize}Y>{\hsize=0.26\hsize}Y@{}}
  \caption{Resources the project required, how each was obtained, and what it cost in cash. The inference figure is read from the provider's billing record; every other line is zero because no money changed hands.}
  \label{tab:budget}\\
  \toprule
  ...
  Hosted model inference & Prepaid balance with a single hosted provider, metered per call. A balance of USD 6.50 was available at handover; USD 2.69 of it had been consumed at the September measurement point, across development, regression runs, specification ingestion and evaluation campaigns. & 2.69 \\
  \addlinespace
  Android test device & One mid-range handset already owned by a team member, with the companion accessibility and input-method application installed (C6). & 0 \\
  \addlinespace
  Development hosts & Five personally owned laptops. One of them additionally ran the emulator, the graph database and the four-process stack. & 0 \\
  \addlinespace
  ...
  \addlinespace
  Engineering effort & Five part-time members over roughly six months. Not priced; see below. & --- \\
  \midrule
  \textbf{Total cash expenditure} & \textbf{Approximately BDT 320.} & \textbf{2.69} \\
  \bottomrule
\end{xltabular}
\endgroup
...
labour that the course absorbed, and its cash cost is under three dollars.
...
line that scales with use, it is worth separating into its two very different
...
the terms of \Cref{sec:economics} and not an operating one. It is also the
project's largest single line item, and the reason it is larger than every
campaign combined is that the document was re-ingested repeatedly while the
ingestion itself was being developed. The metering controls that would cap it
--- an output ceiling, a lower sample count once extraction has stabilised, and
a cache that refuses to re-read an unchanged document --- are identified in
\Cref{sec:conclusion} as outstanding work.

\subsection{Economic Analysis and Financial Prospecting}
\label{sec:economics}

\paragraph{What it cost to build.} USD 2.69 in cash (\Cref{tab:budget}), plus
a session of part-time work by five students. The cash figure is the one worth
quoting and it is the strongest fact in this section: an autonomous exploratory
testing agent was built, debugged and evaluated for under three dollars of
metered inference, because the expensive input to this class of system is
inference, and inference has become cheap enough to be a rounding error rather
than a barrier to entry. The labour is deliberately left unpriced in
...
\begin{xltabular}{\textwidth}{@{}>{\hsize=0.44\hsize}Y>{\hsize=0.32\hsize}Y>{\hsize=1.52\hsize}Y>{\hsize=0.22\hsize}Y@{}}
  ...
  Jonayed Mohiuddin Shanto (2105060) &
    ...
    Generalised the device gateway by removing application-specific prompts and identifiers, which opened the path to DR-03. Built, verified and integrated work packages WP1--WP9 in sequence, the largest single body of delivery in the record. Wrote the browser executor from scratch, bringing the system onto its second platform, and authored the target profiles for both platforms. Fixed the loop-stability, concurrency and run-lock defects, and added the human-in-the-loop CAPTCHA pause. Reclassified a dead-browser condition that had been recorded as an agent navigation fault -- a correction to the attribution taxonomy itself. &
    Apr 2026 \\
  \addlinespace
...
show --- in particular design discussion, review of others' code, and debugging
