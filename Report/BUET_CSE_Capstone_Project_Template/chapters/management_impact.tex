...
Three specifics are added so that the disclosure is checkable rather than
general. First, the assistant was used on this document and not only on the
code: it drafted narrative prose and assembled several of the tables, including
the contribution, budget, model and licence tables, from material the team
supplied or from the repository itself. Second, every factual claim it produced
about this system was verified against a primary source before it was kept:
configuration values against the source files, contribution and chronology
against the version-control record, costs against the provider's billing
record, and dependency licences against the metadata of the versions actually
installed. Several drafted claims were wrong and were corrected by that check,
among them an understated inference cost and a model table built from a stale
example configuration. Third, commits produced with assistance carry a
co-authorship trailer naming the tool, so the boundary between assisted and
unassisted work is recoverable from the repository rather than resting on this
paragraph.

What was not delegated is the substance of the engineering. The architecture,
the requirements analysis and its traceability, the fault-attribution taxonomy,
the choice of what to measure, the interpretation of the measurements, and the
decision about which claims this project is entitled to make are the team's.
The limitations recorded throughout this report, in particular the absence of
precision and recall, were not softened at any point by a tool, and they are
the places where an assistant would most readily have supplied a comfortable
sentence in place of an honest one.
