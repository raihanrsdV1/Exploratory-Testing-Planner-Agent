...
\begin{figure}[htbp]
  ...
  \begin{ganttchart}[
    ...
    \ganttbar{Requirements and feasibility}{2026-01}{2026-03} \\
    \ganttbar{Prototype: planner + executor}{2026-03}{2026-04} \\
    \ganttbar{Generalisation and architecture}{2026-06}{2026-07} \\
    \ganttbar{Work packages WP0--WP9}{2026-07}{2026-08} \\
    \ganttbar{Reliability and fault attribution}{2026-08}{2026-09} \\
    \ganttbar{Web platform and second target}{2026-09}{2026-09} \\
    \ganttbar{Evaluation and report}{2026-08}{2026-09}
  \end{ganttchart}
  \caption{Project schedule}
  \label{fig:gantt}
\end{figure}

\paragraph{Phases.} The work fell into six phases, each ending in a state the
next phase could build on.

\begin{description}[leftmargin=2.2em,style=nextline]
  \item[P1. Requirements and feasibility (January--March).] Analysis of the
    ...
    graph.
  \item[P2. Prototype (March--April).] The repository opened on 19 March 2026.
  ...
  \item[P3. Generalisation (June--July).] Application-specific prompts,
  ...
  \item[P4. Work packages WP0--WP9 (July).] The partner's eight requirement
  ...
  \item[P5. Reliability and fault attribution (August).] Data-integrity
    ...
    measured.
  \item[P6. Second platform, evaluation and report (September).] A browser
...
The project had no institutional compute grant and no procurement budget
(constraint C2). Its resource profile is therefore unusual and worth stating
plainly: almost every input was either free software or effort the course
absorbed, and the only genuine cash outlay was metered model inference.
\Cref{tab:budget} itemises both, separating money actually spent from resources
consumed at no cash cost.

Two cautions apply to the figures. First, every number in the notional block,
and the labour figure in particular, is an \emph{estimate} constructed for this
section rather than a recorded transaction; it is included because an itemised
budget that omits the dominant cost of the project would misrepresent it.
Second, the inference figure is the one number here that is grounded in a
provider's billing record, and its per-test derivation is reported in
\Cref{ch:evaluation}. Conversions use an approximate rate of BDT 120 to the
United States dollar.

\begingroup
\small
\begin{xltabular}{\textwidth}{@{}>{\hsize=0.62\hsize}Y>{\hsize=1.10\hsize}Y>{\hsize=0.28\hsize}Y@{}}
  \caption{Itemised resource budget. Cash figures are actual to within the ranges shown; notional figures are estimates constructed for this section.}
  \label{tab:budget}\\
  \toprule
  Resource & Basis for the estimate & Cost (USD) \\
  \midrule
  \endfirsthead
  \caption[]{Itemised resource budget \emph{(continued)}}\\
  \toprule
  Resource & Basis for the estimate & Cost (USD) \\
  \midrule
  \endhead
  ...
  \multicolumn{3}{@{}l}{\itshape Cash outlay} \\
  \addlinespace[2pt]
  Hosted model inference & Prepaid balance with a hosted inference provider, across development, regression runs and campaigns. Flagship tier for planning, cheaper tier for execution and investigation after the quality cost was measured. & 60--120 \\
  \addlinespace
  Electricity and connectivity & Marginal share attributable to the project across five members over roughly seven months. & 40--80 \\
  \addlinespace
  \textbf{Subtotal, cash} & & \textbf{100--200} \\
  \midrule
  \multicolumn{3}{@{}l}{\itshape Consumed at no cash cost (notional)} \\
  \addlinespace[2pt]
  Engineering effort & Five part-time members, roughly 1{,}300 person-hours over the session, valued at a junior software-engineering rate of about USD 3 per hour for this market. Dominates every other line by an order of magnitude. & $\approx$ 3{,}900 \\
  \addlinespace
  Development hosts & Five personally owned laptops, amortised over the project period. One host additionally ran the Android emulator and the four-process stack. & $\approx$ 500 \\
  \addlinespace
  Android test device & One mid-range handset, already owned, with the companion accessibility and input-method application installed (C6). & $\approx$ 150 \\
  \addlinespace
  Graph database, web and service frameworks, browser and device drivers, embedding and document-handling libraries & Open-source components, listed with their licences in \Cref{sec:ip}. Self-hosted; no licence fee. & 0 \\
  \addlinespace
  Version control, continuous integration, observability, design export & Free tiers of hosted developer services. & 0 \\
  \addlinespace
  Application under test and its specification & Supplied by the supervisor and the industry partner; a production application with live users, and a requirements document. & 0 \\
  \addlinespace
  \textbf{Subtotal, notional} & & $\approx$ \textbf{4{,}550} \\
  \midrule
  \textbf{Total economic cost} & & $\approx$ \textbf{4{,}700} \\
  \bottomrule
\end{xltabular}
\endgroup

The shape of this table is the finding, not the total. Better than nine tenths
of the project's economic cost is human time, and the part that scales with
\emph{use} rather than with development is the inference line alone. That is
the property the next two subsections turn on: a system whose marginal cost per
unit of work is cents, and whose fixed cost is a one-time engineering
investment, has a different economic argument available to it than one that
bills per seat.

\subsection{Economic Analysis and Financial Prospecting}
\label{sec:economics}

\paragraph{What it cost to build.} Approximately USD 4{,}700 of economic cost,
of which USD 100--200 was cash (\Cref{tab:budget}). A commercial team would not
reproduce this figure: five part-time students over a session is not two
full-time engineers over three months, and a firm paying market salaries with
overhead should expect a first production-grade version of this system to cost
an order of magnitude more. The honest claim is about the \emph{cash} figure,
and it is a strong one: the entire metered cost of building and evaluating an
autonomous exploratory testing agent was under two hundred dollars, because the
expensive input to this class of system is inference and inference is now
cheap enough to be a line item rather than a barrier.
...
executed test case, reported with its derivation in \Cref{ch:evaluation} and
...
Meituan~\cite{auitestagent}. Valued at the rate used in \Cref{tab:budget},
1.5 person-days is roughly USD 36; valued at a mid-level engineer's fully
loaded cost in a market like Singapore or Seoul it is several hundred dollars.
...
year, against an inference bill measured in tens of dollars. The ratio is
...
\begingroup
\small
\begin{xltabular}{\textwidth}{@{}>{\hsize=0.52\hsize}Y>{\hsize=0.42\hsize}Y>{\hsize=1.32\hsize}Y>{\hsize=0.24\hsize}Y@{}}
  \caption{Member roles, principal contributions and delivery against schedule}
  \label{tab:contrib}\\
  \toprule
  Member & Role & Principal contributions & On time \\
  \midrule
  \endfirsthead
  \caption[]{Member roles and contributions \emph{(continued)}}\\
  \toprule
  Member & Role & Principal contributions & On time \\
  \midrule
  \endhead
  ...
  Jonayed Mohiuddin Shanto (2105060) &
    Execution and integration lead &
    Generalised the device gateway by removing application-specific prompts and identifiers, which opened the path to DR-03. Built, verified and integrated work packages WP1--WP9 in sequence. Wrote the browser executor from scratch, bringing the system onto its second platform. Authored target profiles for both platforms. Fixed the loop-stability, concurrency and run-lock defects, and added the human-in-the-loop CAPTCHA pause. Reclassified a dead-browser condition that had been recorded as an agent navigation fault, which was a correction to the attribution taxonomy itself. &
    Yes \\
  \addlinespace
  Mehemud Azad (2105014) &
    Planner and retrieval &
    Integrated the executor with the planner to close the first end-to-end loop. Built the goal-based planner and the trajectory evaluator. Owned the retrieval service and the embedding backend, and rebuilt the orchestration on a graph-execution framework. Added the observability layer. Carried out the migration to a cheaper model tier and added graph backup and restore. Contributed the largest share of the project's internal documentation. &
    Yes \\
  \addlinespace
  Mohammad Raihan Rashid (2105046) &
    Architecture and repository owner &
    Opened the repository and established the four-process structure. Removed the hard-coded ingestion path and made the pipeline modular. Centralised configuration and fixed the state-identity and observation defects underlying screen recognition. Made the agent's own failures visible as a distinct class, which is the work behind NFR-D3 and the precondition for any defensible measurement. Reviewed and merged the team's pull requests. &
    Yes \\
  \addlinespace
  Khalid Hasan Tuhin (2105002) &
    Operator surface and web target &
    Built the target-configuration interface and the knowledge-upload path in the dashboard, which is what makes DR-09 an operator action rather than a code change. Contributed to the browser executor, adding the wandering guard, risk-priority wiring, agent-difficulty steering and semantic retrieval of execution notes. Configured the production commerce target and produced the campaign report. Fixed the executor's crash on provider rate-limiting and a precondition misclassification. &
    Yes \\
  \addlinespace
  Niloy Kumar Mondal (2105044) &
    Literature grounding and internal review &
    Designed the run-report agent that closes the campaign loop into a document
    a stakeholder can act on: it gathers execution logs, findings and coverage
    state from the knowledge graph, filters outcomes against the
    fault-attribution taxonomy so that only application-caused failures are
    reported as defects, and renders the result as a downloadable,
    human-readable PDF carrying a generated narrative (DR-08); also corrected
    a defect in its handling of long campaign batches. Surveyed fifteen recent
    related works and assessed each for relevance, which is the material
    behind \Cref{subsec:additional}. Carried out the structured internal code
    review against an explicit severity rubric referred to in
    \Cref{sec:accountability}. Ran testing against the second target. &
    Yes \\
\end{xltabular}
\endgroup
...
before submission, and add any contribution the version-control record does not
show, in particular design discussion, review of others' code, and time spent
debugging that ended in no commit. If any responsibility was \emph{not}
completed on time, say so and say why; a table in which every entry reads
``yes'' is less credible than one that records a slip honestly.}
...
\guidance{TODO (team): if any disagreement arose that was not technical, or any
that took real effort to resolve, describe it here with what was done. This
subsection is assessed on evidence of conflict being identified and resolved,
and an account containing only smooth technical decisions is weaker than one
that reports a genuine difficulty and how the team came through it.}
...
\paragraph{An applied domain with live users.} The principal evaluation target
is a production agricultural-commerce application with real farmers and sellers
...
\guidance{TODO (team): if the team met the partner's engineers, the supervisor
or any prospective user in a scheduled review, record when, who took part and
what changed as a result. A dated engagement is stronger evidence for this
indicator than a description of the disciplines involved.}
...
\begin{xltabular}{\textwidth}{@{}>{\hsize=0.70\hsize}Y>{\hsize=0.86\hsize}Y>{\hsize=0.44\hsize}Y@{}}
  \caption{Principal third-party components and their licences}
  \label{tab:licences}\\
  \toprule
  ...
  Android execution driver & Device execution through the accessibility service & Permissive; see note \\
  \addlinespace
  fastembed, NumPy & Embeddings and numerical support & Apache-2.0, BSD-3 \\
  markitdown, pypdf & Format-agnostic document ingestion & MIT, BSD-3 \\
  fpdf2 & Run-report PDF generation & LGPL-3.0 \\
  \addlinespace
  ...
  \addlinespace
  React, Vite & Operator dashboard & MIT \\
  \addlinespace
...
\guidance{TODO (team): before submission, regenerate this table from the
environment actually used (\texttt{pip-licenses --format=markdown} and
\texttt{license-checker} for the dashboard), confirm the Android execution
driver's licence from its own repository rather than from this table, and state
the licence the team intends to release its own code under.}
...
concealed. The team used an agentic coding assistant during implementation,
under a repository-level instruction file that the team wrote and maintained.
Its use fell into four areas: drafting and refactoring implementation code
within a design the team had already fixed; generating and maintaining internal
documentation; assisting with the narrative prose of this report; and producing
the generated summary text inside the system's own exported run report, where
the numbers are computed from the knowledge graph and the model supplies
connecting prose only.

The boundary the team held to is that judgement was not delegated. The
four-process architecture, the decision that the components communicate only
through a persistent graph, the requirements analysis and its traceability, the
fault-attribution taxonomy, the choice of what to measure and the
interpretation of what the measurements mean are the team's own. Generated code
was read, executed and tested before it was kept, and several of the defects
recorded in \Cref{ch:evaluation} were defects in generated code that review and
testing caught. No claim in this report was accepted because a model asserted
it: the empirical figures attributed to other systems in \Cref{ch:research} were
read from the papers themselves.

\guidance{TODO (team): check the wording above against the disclosure statement
the department requires, and amend it to match. If any member used these tools
in a way this paragraph does not describe, add it; an incomplete disclosure is
worse than a broad one.}
