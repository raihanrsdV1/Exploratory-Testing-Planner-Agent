...
\guidance{TODO (team): the three works cited in this subsection
(\texttt{androidworld}, \texttt{appworld}, \texttt{digirl}) were surveyed from
their published abstracts and metadata by a team member, not read in full, and
their \texttt{references.bib} entries use first-author-et-al form. Verify all
three against the published versions before submission, and either read the
visual-grounding and survey works and cite them properly or leave them uncited
as they are here. This caveat does \emph{not} apply to the works of
\Cref{subsec:formative}, which were read in full; of those, only
\texttt{ares} needs a bibliographic check, since the copy consulted was the
arXiv preprint (2101.02636) and the volume, issue and article number of the
published TOSEM version must be filled in.}
