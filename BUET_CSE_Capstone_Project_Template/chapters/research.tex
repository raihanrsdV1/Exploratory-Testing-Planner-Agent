...
Seven primary systems were read in full and form the evidentiary basis of this
review: four are GUI \emph{testing}
systems~\cite{gptdroid,droidagent,auitestagent,xuatcopilot} and three are GUI
\emph{task-automation} agents~\cite{autodroid,appagent,appagentx} whose memory
and grounding mechanisms transfer directly to the testing problem. Every figure
quoted below was taken from those papers. The review is organised around four
questions: how the field arrived at language-model agents, what those agents
demonstrably add, how the resulting claims are measured, and what the
...
parallel line applies reinforcement learning with curiosity rewards to reach
novel states.
...
roles~\cite{droidagent}, and AUITestAgent makes the separation of interaction
from verification its central design claim~\cite{auitestagent}.
...
\begin{enumerate}[label=\textbf{G\arabic*.},leftmargin=3.0em]
  \item \textbf{No specification is ever ingested.} Not one of the seven systems
    reads a requirements document, builds a requirement graph, or reports
    requirement coverage. AUITestAgent comes closest by accepting one
    natural-language test requirement per test, but it holds no persistent model
    of the requirement set and cannot say which requirements remain untested.
  \item \textbf{Memory does not leave the application or the session.}
    ...
    another.
  \item \textbf{Defect history is never a knowledge source.} None of the seven
    reads a bug tracker, and none biases exploration toward historically fragile areas, despite defect density being among the oldest and best-established
    predictors in software engineering.
...
Beyond the seven systems read in full, the team surveyed a wider set of recent
...
\guidance{TODO (team): the three cited works (androidworld, appworld, digirl)
have been verified against their published versions (ICLR/ACL/NeurIPS
proceedings) and \texttt{references.bib} now carries full author lists in
place of first-author-et-al form; the numeric claims made about them in this
subsection were checked against the same sources and are unchanged. Still
open: the visual-grounding and screenshot-only action models, and the
operating-system-agent review, named in the paragraph above are not cited to a
specific paper. Either identify and read the specific works surveyed and cite
them properly, or delete this note and leave the paragraph uncited as it
stands.}
...
\begin{xltabular}{\textwidth}{@{}>{\hsize=0.58\hsize}Y>{\hsize=1.06\hsize}Y>{\hsize=1.36\hsize}Y@{}}
  ...
  \addlinespace
  The partner's existing system & The static-knowledge generator described in ETA-TR-2026-001 \S2 \cite{etatr} &
...
agricultural-commerce application was supplied by the supervisor as a second
...
\guidance{TODO (team): if a structured interview with the partner's QA engineers
or a practitioner survey is conducted before submission, report its method,
sample and findings here; that would upgrade this section from secondary
evidence to primary. Also add any market-size figure the team can cite
properly.}
